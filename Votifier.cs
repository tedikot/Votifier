...
namespace unturned.ROCKS.Votifier
{
    public class Votifier : RocketPlugin
    {
        ...
        {
            string unturnedServer = String.IsNullOrEmpty(Votifier.Configuration.UnturnedServers) ? "" : new WebClient().DownloadString(String.Format("http://unturned-servers.net/api/?object=votes&element=claim&key={0}&steamid={1}", Votifier.Configuration.UnturnedServers, id.ToString()));
            string unturnedSL = String.IsNullOrEmpty(Votifier.Configuration.UnturnedSL) ? "" : new WebClient().DownloadString(String.Format("http://unturnedsl.com/api/dedicated/{0}/{1}", Votifier.Configuration.UnturnedSL, id.ToString()));
